...
void lv_demo_widgets(void)
{
    ...
    lv_style_init(&style_title); //初始化title样式
    lv_style_set_text_font(&style_title, font_large); //字体样式大小设置

    lv_style_init(&style_icon); //初始化icon样式
    lv_style_set_text_color(&style_icon, lv_theme_get_color_primary(NULL)); //设置icon字体颜色
    lv_style_set_text_font(&style_icon, font_large); //设置 icon 字体大小

    lv_style_init(&style_bullet); //
    lv_style_set_border_width(&style_bullet, 0);
    lv_style_set_radius(&style_bullet, LV_RADIUS_CIRCLE);

    tv = lv_tabview_create(lv_screen_active()); //在屏幕上创建空的选项卡视图
    lv_tabview_set_tab_bar_size(tv, tab_h);  //设置选项卡大小
    lv_obj_add_event_cb(tv, tabview_delete_event_cb, LV_EVENT_DELETE, NULL); //添加相关事件

    lv_obj_set_style_text_font(lv_screen_active(), font_normal, 0); //字体大小设置

    lv_obj_t * t1 = lv_tabview_add_tab(tv, "Profile"); //添加选型卡
...
static void profile_create(lv_obj_t * parent)
{
    ...
    LV_IMAGE_DECLARE(iris_unlock_main); //声明图片文件（这里是.c文件）
    lv_obj_t * avatar = lv_image_create(panel1); //在panel1上面创建图片
    //lv_image_set_src(avatar, &img_demo_widgets_avatar);
    
    lv_image_set_src(avatar, &iris_unlock_main); //设置图片的源
    //lv_img_set_angle(&img_test, 900);
    //lv_image_set_src(avatar, "./lvgl/demos/widgets/assets/avatar.png");
    //lv_image_set_src(avatar, "./avatar.png");

    lv_obj_t * name = lv_label_create(panel1); //在panel1 上面创建一个lable
    lv_label_set_text(name, "Elena Smith"); //
    lv_obj_add_style(name, &style_title, 0); //给label添加相关的样式
